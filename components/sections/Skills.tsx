import { BarChart3, Lightbulb, PenTool, Wrench } from "lucide-react";
...
/** Four groups, two to a row on a wide screen. */
const skillGroups = [
  { title: "Product", icon: Lightbulb, items: skills.product },
  { title: "Design", icon: PenTool, items: skills.design },
  { title: "Data & Analytics", icon: BarChart3, items: skills.data },
  { title: "Tools & Growth", icon: Wrench, items: skills.tools },
];
...
function SkillChip({ skill }: { skill: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-md border border-border bg-surface-hover px-3 py-2 text-label font-medium text-muted-foreground transition-colors hover:border-brand/40 hover:text-foreground">
    ...
    >
      ...
      <BlurFade inView delay={0.2}>
        <div className="relative mt-14 flex w-full flex-col gap-4 overflow-hidden py-2">
          {/* One band: with nine marks, a second running the other way would
              only show the same ones again. */}
          <Marquee pauseOnHover className="[--duration:40s] [--gap:4rem]">
            {marqueeLogos.map((logo) => (
              <TechLogo key={logo.src} logo={logo} className="size-10" />
            ))}
          </Marquee>
      ...
      <div className="mx-auto mt-14 grid max-w-7xl gap-5 px-6 md:grid-cols-2">
        {skillGroups.map((group, index) => (
          <BlurFade key={group.title} inView delay={0.1 * index}>
