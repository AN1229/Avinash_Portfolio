 ...
 *
 * Each role's node is one fixed-size tile holding the company's logo, or its
 * initials until there is a logo to use.
 */
...
/** "Thermo Fisher Scientific" → "TF": the first two words, skipping legal suffixes. */
function initialsOf(company: string) {
  return company
    .split(" ")
    .filter((word) => !/^(private|limited|pvt|ltd|llp|inc)\.?$/i.test(word))
    .slice(0, 2)
    .map((word) => word[0])
    .join("")
    .toUpperCase();
...
export function Experience() {
  ...
  return (
    ...
    //
    // Banded, alternating with the unbanded Projects section above it.
    <section
      id="experience"
          ...
          {experience.map((role, index) => {
            ...
            return (
              ...
              >
                ...
                marked. A logo is contained inside the tile rather than filling
                it, since marks come in very different shapes, on a white
                backing so a dark-on-transparent mark stays visible. */}
                ...
                <span
                  className={cn(
                    "absolute left-0 top-0 z-10 flex size-14 items-center justify-center overflow-hidden rounded-xl border ring-4 ring-surface transition-[border-color,box-shadow] duration-500 md:size-[72px]",
                    role.logo ? "bg-white" : "bg-background",
                    index <= reached
                ...
                >
                  {role.logo ? (
                    // As large as the tile allows, less a thin margin so the
                    // mark does not touch the border. Contained rather than
                    // cropped, so a wide wordmark keeps its ends.
                    ...
                    <span
                      ...
                      className={cn(
                        ...
                        index <= reached ? "text-brand" : "text-foreground/80",
                      )}
                    >
                ...
                <BlurFade inView delay={0.05 * index}>
                  ...
                  loudest thing on every entry the least interesting: what the
                  role was and who for should be read first. They are still directly
                  ...
                  <p className="mt-1 font-heading text-lg font-semibold text-brand md:text-xl">
                    {role.company}
                  </p>
                  ...
                  {role.summary && (
                    <p className="mt-4 text-body italic text-foreground/85">
                      {role.summary}
                    </p>
                  )}
                  ...
                  <ul className="mt-5 space-y-3">
                    {role.bullets.map((bullet) => (
                      ...
                      >
                        <span className="mt-[0.8em] size-1.5 shrink-0 rounded-full bg-border" />
                        {bullet}
                      </li>
                    ))}
                  </ul>
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-4">
                      {roleLinks.map((link) => (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-meta font-medium text-brand transition-colors hover:text-brand-hover"
                        >
                          {link.label}
